...
async function getUser(req, res, next) {
    ...
    if ((req.header("Authorization") && !req.header("Authorization").startsWith("Basic")) || req.body.access_token) {
        ...
        if (active) {
            ...
            req.user = { user_id: results[0].user_id, username: results[0].username, email: results[0].email, permissions: await getPermissions(results[0].user_id, results[0].client_id), admin: hasPermission(results[0].user_id, await dbInterface.getDashboardId(), "admin"), origin: "access_token" };
        } else {
        ...
        if (results.length === 1) {
            ...
            req.user = { user_id: results[0].user_id, username: results[0].username, email: results[0].email, permissions: await getPermissions(results[0].user_id, results[0].client_id), admin: hasPermission(results[0].user_id, await dbInterface.getDashboardId(), "admin"), origin: "refresh_token" };
        } else {
        ...
        if (results.length === 1) {
            ...
            req.user = { user_id: results[0].user_id, username: results[0].username, email: results[0].email, permissions: await getPermissions(results[0].user_id, results[0].client_id), admin: hasPermission(results[0].user_id, await dbInterface.getDashboardId(), "admin"), origin: "authorization_code" };
        } else {
        ...
        if (user_id) {
            ...
            if (results.length === 1)
                req.user = { user_id: user_id, username: results[0].username, email: results[0].email, admin: hasPermission(results[0].user_id, await dbInterface.getDashboardId(), "admin"), origin: "basic" }
        } else {
