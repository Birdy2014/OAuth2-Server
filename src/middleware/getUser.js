...
async function getUser(req, res, next) {
    try {
        ...
        if ((req.header("Authorization") && !req.header("Authorization").startsWith("Basic")) || req.body.access_token) {
            ...
            let results = await dbInterface.query(`SELECT access_token.user_id AS user_id, access_token.expires AS expires, access_token.client_id AS client_id, client.name AS client_name FROM access_token JOIN client ON access_token.client_id = client.client_id WHERE access_token.access_token = '${access_token}'`);
            ...
            let results = await dbInterface.query(`SELECT refresh_token.user_id AS user_id, refresh_token.client_id AS client_id, refresh_token.expires AS expires, client.name AS client_name FROM refresh_token JOIN client ON refresh_token.client_id = client.client_id WHERE refresh_token = '${refresh_token}'`);
            if (results.length > 0 && results[0].expires > currentUnixTime()) {
                req.client = { client_id: results[0].client_id, name: results[0].client_name, origin: "refresh_token" }
                req.user = await getUserInfo(results[0].user_id);
                req.user.permissions = await getPermissions(results[0].user_id, results[0].client_id);
                req.user.origin = "refresh_token";
                dbInterface.query(`UPDATE refresh_token SET expires = '${currentUnixTime() + configReader.config.refreshTokenExpirationTime}' WHERE refresh_token = '${refresh_token}'`)
            } else {
            ...
            let results = await dbInterface.query(`SELECT authorization_code.user_id AS user_id, authorization_code.client_id AS client_id, authorization_code.expires AS expires, client.name AS client_name FROM authorization_code JOIN client ON authorization_code.client_id = client.client_id WHERE authorization_code.authorization_code = '${authorization_code}'`);
            if (results.length > 0 && results[0].expires > currentUnixTime()) {
                ...
                dbInterface.query(`DELETE FROM authorization_code WHERE authorization_code = '${authorization_code}'`);
            } else {
